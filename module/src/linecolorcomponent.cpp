...
RTTI_BEGIN_CLASS(nap::LineColorComponent)
	RTTI_PROPERTY("Line",					&nap::LineColorComponent::mLine,				nap::rtti::EPropertyMetaData::Default)
...
namespace nap
{
	bool LineColorComponentInstance::init(utility::ErrorState& errorState)
	{
		...
		mPolyLine = mResource->mLine.get();
		if (!errorState.check(mResource->mLine != nullptr, "Line not set"))
			return false;
	...
	void LineColorComponentInstance::update(double deltaTime)
	{
		...
		nap::Vec4VertexAttribute& color_attr = mPolyLine->getColorAttr();
		...
		if (!mPolyLine->getMeshInstance().update(error))
		{
